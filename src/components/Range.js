...
class Range {
    ...
    constructor(
        ...
        this.inputRange.style.setProperty(
            "--val",
            +this.inputRange.style.getPropertyValue("--val")
        );
    ...
    init() {
        this.inputRange.addEventListener(
            ...
            e => {
                ...
                let currentValue =
                    parseInt(this.inputRange.value) === 0
                        ? 1
                        : this.inputRange.value;

                // Insert the current value into the input target regarding the round rule.
                if (this.round) {
                    document.querySelector(this.inputBox).value = Math.ceil(
                        (currentValue * this.maxNumber) / 100
                    );
                } else {
                    document.querySelector(this.inputBox).value =
                        (currentValue * this.maxNumber) / 100;
                }
            },
            false
